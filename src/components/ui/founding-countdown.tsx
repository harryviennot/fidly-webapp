...
// Cache by value-key so the same reference is returned until the displayed
// values actually change — without this we'd return a fresh object on every
// call and trigger an infinite render loop.
...
/**
 * Live countdown to the founding-partner program close
 * (`FOUNDING_PROGRAM_END_DATE` in `lib/pricing.ts`). Returns null after the
 * cutoff so callers can render the component unconditionally and let it
 * disappear server-side.
 *
 * `inline` is a plain accent-coloured line; `badge` is a pill with a pulsing
 * dot — the badge variant is the one the onboarding plan step uses above
 * the tier grid.
 */
export function FoundingCountdown({ variant = 'inline', className = '' }: Props) {
  ...
  return (
    <span className={`text-sm font-semibold text-[var(--accent)] ${className}`}>
      {label}
    </span>
  );
}
