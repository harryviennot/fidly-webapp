...
export function StepIllustration({ step, done, active, className }: StepIllustrationProps) {
    ...
    if (step === 2) return (
        <svg width="100%" height="100%" viewBox="0 0 80 80" fill="none" className={cn("overflow-visible", className)}>
            <style>{`
        .s2-wand { transform-origin: 54px 30px; animation: s2-paint 2s infinite alternate ease-in-out; }
        .s2-stamp { opacity: 0; animation: s2-pop 1.5s infinite alternate ease-in-out; }
        .s2-stamp-2 { animation-delay: 0.5s; }
        .s2-stamp-3 { animation-delay: 1s; }
        @keyframes s2-paint { 
      ...
      `}</style>
            ...
            <circle cx="29" cy="38" r="1.5" fill={accent} className={cn(active && !done && "s2-stamp")} />
            <circle cx="34" cy="38" r="1.5" fill={accent} className={cn(active && !done && "s2-stamp s2-stamp-2")} />
            <circle cx="39" cy="38" r="1.5" fill={accent} className={cn(active && !done && "s2-stamp s2-stamp-3")} />
            <circle cx="44" cy="38" r="1.5" fill="#EEEDEA" stroke="#DDD" strokeWidth="0.5" />
            {done ? (
                <g className="s1-check" style={{ transformOrigin: '54px 30px', animation: 's1-pop 0.5s forwards' }}>
    ...
    if (step === 3) return (
        <svg width="100%" height="100%" viewBox="0 0 80 80" fill="none" className={cn("overflow-visible", className)}>
            <style>{`
        .s3-card { transform-origin: 40px 40px; }
        .s3-float { animation: s3-float 3s infinite ease-in-out; }
        .s3-wave { transform-origin: 40px 40px; animation: s3-radiate 2.5s infinite ease-out; opacity: 0; }
        .s3-wave-2 { animation-delay: 0.8s; }
        .s3-wave-3 { animation-delay: 1.6s; }
        @keyframes s3-float { 
          0%, 100% { transform: translateY(0px); } 
          50% { transform: translateY(-4px); } 
        }
        @keyframes s3-radiate { 
      ...
      `}</style>
            ...
            {active && !done && (
                <g>
                    <circle cx="40" cy="40" r="16" stroke={accent} fill="none" className="s3-wave" />
                    <circle cx="40" cy="40" r="16" stroke={accent} fill="none" className="s3-wave s3-wave-2" />
                    <circle cx="40" cy="40" r="16" stroke={accent} fill="none" className="s3-wave s3-wave-3" />
                </g>
            )}

            <g className={cn("s3-card", active && !done && "s3-float")}>
            ...
            {done && (
                <g className="s1-check" style={{ transformOrigin: '55px 45px', animation: 's1-pop 0.5s forwards' }}>
    ...
    if (step === 4) return (
        <svg width="100%" height="100%" viewBox="0 0 80 80" fill="none" className={cn("overflow-visible", className)}>
            <style>{`
        .s4-bell { transform-origin: 40px 24px; }
        .s4-ring { animation: s4-ring 2.5s infinite ease-in-out; }
        .s4-pulse { animation: s4-pulse 2.5s infinite ease-in-out; }
        .s4-pulse-2 { animation-delay: 0.2s; }
        @keyframes s4-ring { 
        ...
        @keyframes s4-pulse { 
          ...
          15%, 25% { opacity: 1; transform: scale(1) translateY(0px); }
          40% { opacity: 0; transform: scale(1.1) translateY(-5px); }
        }
      `}</style>
            <circle cx="40" cy="40" r="36" fill={bg} />
            ...
            {active && !done && (
                <g>
                    <path d="M50 22 a8 8 0 0 1 4 8" fill="none" stroke={accent} strokeWidth="2" strokeLinecap="round" className="s4-pulse" />
                    <path d="M26 22 a8 8 0 0 0 -4 8" fill="none" stroke={accent} strokeWidth="2" strokeLinecap="round" className="s4-pulse s4-pulse-2" />
                </g>
            )}

            <g className={cn("s4-bell", active && !done && "s4-ring")}>
                <path d="M40 24 c-5.5 0-9 4-9 10 v6 l-4 4 v2 h26 v-2 l-4-4 v-6 c0-6-3.5-10-9-10 z" fill="#fff" stroke={base} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                <path d="M37 48 a3 3 0 0 0 6 0" fill="none" stroke={base} strokeWidth="2" strokeLinecap="round" />

                {(!done || active) && (
                    <circle cx="50" cy="24" r="5" fill={accent} stroke="#fff" strokeWidth="1.5" />
                )}
            </g>

            {done && (
                <g className="s1-check" style={{ transformOrigin: '55px 48px', animation: 's1-pop 0.5s forwards' }}>
                    <circle cx="55" cy="48" r="10" fill={base} />
                    <path d="M51 48l2.5 2.5L59 45" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                </g>
            )}
