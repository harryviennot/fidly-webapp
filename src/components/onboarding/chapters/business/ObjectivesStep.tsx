...
const OPTIONS = ['retention', 'frequency', 'basket', 'acquisition'] as const;

/**
 * Chapter 2 sub-step — optional. Saves to `settings.primary_goal`. Single-
 * select for v1: users pick the primary goal. Multi-select can come later if
 * analytics needs it.
 */
export function ObjectivesStep() {
  const t = useTranslations('onboardingBusiness.chapters.business.steps.objectives');
    ...
    ctx.setSubmitHandler(async () => {
      ...
      try {
        await updateBusiness({
          settings: { ...(currentBusiness.settings ?? {}), primary_goal: selected },
        });
        return { ok: true };
      } catch (err) {
